...
class RulesProcessor:
    ...
    def check_matches(self, messages: list) -> (list, list):
        ...
        if len(messages) > 0:
            for message in messages:
                ...
                self.logger.debug(
                    "Rules match results for message:\n"
                    + "\tuuid: %s\n"
                    + "\tfrom: %s\n"
                    + "\tsubject: %s\n"
                    + "\tmatched: %s\n"
                    ...
                    message.subject,
                    results.matched,
                    results.rule.name if results.rule else None,
                )
    ...
    def _is_match(self, message: EmailStruct) -> MatchResult:
        ...
        for rule in self.rules:
            ...
            if (
                rule.from_ is not None
                and rule.subject is not None
                and rule.from_ in message.from_
                and rule.subject in message.subject
            ):
                matched = True
        ...
        return MatchResult(matched=matched, rule=matched_rule, message=message)
