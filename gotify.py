...
class Gotify:
            ...
            for match in matchResults:
                if match.matched:
                    ...
                    if response:
                        ...
                        num_failed+=1
            self.logger.info("Gotify send results: %d sent sucessfully, %d failed to send", len(sent_messages), num_failed)
    ...
    def _send_to_gotify(self,message,rule):
        ...
        if response.status_code != 200:
            self.logger.error("Failed to send message UUID: %s to Gotify, status code: %d",
                            message.msgid, response.status_code)
            return False

        self.logger.debug("Send of email message UUID: %s to gotify with priority: %d was successful",
                            message.msgid, priority)
